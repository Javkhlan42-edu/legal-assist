...
const envSchema = z
  .object({
    ...
    RETRIEVAL_SPEED_MODE: z.enum(['fast', 'balanced', 'quality']).default('balanced'),
    INCLUDE_RELATED_CASES: z.enum(['auto', 'always', 'never']).default('auto'),
    RETRIEVAL_TIMEOUT_MS: z.coerce.number().int().positive().default(35000),
    GENERATION_TIMEOUT_MS: z.coerce.number().int().positive().default(35000),
    RESPONSE_LATENCY_BUDGET_MS: z.coerce.number().int().positive().default(70000),
    RETRIEVAL_CACHE_ENABLED: booleanFromEnv.default(true),
    RETRIEVAL_CACHE_TTL_SECONDS: z.coerce.number().int().positive().default(86400),
    RETRIEVAL_CACHE_MIN_QUALITY: z.coerce.number().min(0).max(1).default(0.55),
    RETRIEVAL_CACHE_VERSION: z.string().default('retrieval-context-v1'),
