...
const envSchema = z
  .object({
    ...
    OPENAI_TIMEOUT_MS: z.coerce.number().int().positive().default(180000),
    RETRIEVAL_SPEED_MODE: z.enum(['fast', 'balanced', 'quality']).default('quality'),
    INCLUDE_RELATED_CASES: z.enum(['auto', 'always', 'never']).default('auto'),
    RETRIEVAL_TIMEOUT_MS: z.coerce.number().int().positive().default(180000),
    GENERATION_TIMEOUT_MS: z.coerce.number().int().positive().default(180000),
    RESPONSE_LATENCY_BUDGET_MS: z.coerce.number().int().positive().default(240000),
